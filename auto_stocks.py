"""
Indian stock market pipeline — runs every 5 hours via GitHub Actions.
Fetches top trending Indian stock story, generates Hindi video, uploads to YouTube.
"""
...
from script_gen_hi import generate_hindi_stock
...
def main():
    print("=" * 60)
    print("  INDIAN STOCKS — HINDI AUTO PIPELINE")
    ...
    print("\n[1] Fetching top Indian stock stories...")
    stories = get_top_stories(n=15)
    ...
    story = fresh[0]
    print(f"  Selected : [{story['score']}] {story['title'][:70]}")
    print(f"  Source   : {story['src']}")

    print("\n[2] Generating Hindi script + SEO via Claude Sonnet...")
    try:
        script = generate_hindi_stock(story["title"], story.get("desc", ""))
    ...
    audio_path  = os.path.join(CACHE_DIR,  f"stocks_{slug}_audio.mp3")
    output_path = os.path.join(OUTPUT_DIR, f"stocks_{slug}_short.mp4")
